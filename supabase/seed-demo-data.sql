...
BEGIN
  ...
  INSERT INTO public.jobs (company_id, title, location, salary, type, description, requirements, benefits, applicant_count, status, deadline) VALUES
    ...
    -- 한양건설 - 마감
    (c1_id, '잠실 주상복합 배관 시공', '서울 송파구', '일 38만원', '장기 (3개월)', '잠실 신축 주상복합 배관 시공 완료. 감사합니다.', ARRAY['배관공 경력 7년 이상','대형 현장 경험'], ARRAY['4대보험','중식제공','기숙사'], 28, 'closed', NOW() - INTERVAL '10 days'),
    ...
    -- 테크파크건설 - 마감
    (c2_id, '분당 오피스 LED 조명 교체', '경기 성남시', '일 32만원', '단기 (3일)', '분당 오피스빌딩 전층 LED 조명 교체 완료.', ARRAY['조명설치 경험 2년 이상'], ARRAY['식대 제공'], 15, 'closed', NOW() - INTERVAL '5 days'),
    ...
    -- 대한인테리어 - 마감
    (c3_id, '성수동 사무실 리모델링', '서울 성동구', '일 35만원', '단기 (2주)', '성수동 사무실 리모델링 공사 완료.', ARRAY['인테리어 경력 5년 이상','다기능 시공 가능자'], ARRAY['4대보험','식대'], 20, 'closed', NOW() - INTERVAL '15 days'),
    ...
    -- 서울플랜트 - 마감
    (c4_id, '인천 공장 설비 배관 공사', '인천 남동구', '일 50만원', '장기 (3개월)', '인천 산업단지 공장 설비 배관 공사 완료.', ARRAY['배관 용접 경력 8년 이상','산업설비 경험'], ARRAY['4대보험','기숙사','통근버스'], 32, 'closed', NOW() - INTERVAL '20 days');
  ...
  RAISE NOTICE '기술자 6명, 기업 4명, 관리자 1명, 공고 11건 생성';
